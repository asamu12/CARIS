...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, image_features, text_features, attention_mask=None):
        image_features = self.image_proj(image_features)  # (batch_size, seq_len, dim)
        text_features = self.text_proj(text_features)  # (batch_size, seq_len, dim)

        query = image_features.permute(1, 0, 2)  # (seq_len, batch_size, embed_dim)
        key = text_features.permute(1, 0, 2)  # (seq_len, batch_size, embed_dim)
...
class MultiBranchFusionAdapter(nn.Module):
    def __init__(
            self,
            fc_in_channels: int,
            in_channels: int,
