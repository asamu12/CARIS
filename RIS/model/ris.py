...
from RIS.model.clip import build_model
from .layers import Neck, Decoder, Projector, ImageTextFusion
...
class CARIS(nn.Module):
    def __init__(self, cfg):
        ...
        self.txt_backbone = build_model(clip_model.state_dict(), cfg.word_len, cfg.input_size, cfg.txtual_module_layer, cfg.txt_module_dim).float()
        self.fusion = ImageTextFusion(d_model=cfg.ladder_dim, nhead=cfg.nhead, dino_layers=cfg.dino_layers, output_dinov2=cfg.output_dinov2)
        ...
        if cfg.dino_name == 'dino-base':
            self.dinov2 = vit_base(
                ...
                init_values=1.0,
                block_chunks=0,
                add_module_layer=cfg.visual_module_layer,
                visual_module_dim=cfg.visual_module_dim,
            )
        else:
