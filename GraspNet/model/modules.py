...
class CloudCrop(nn.Module):
    ...
    def forward(self, seed_xyz, pointcloud, vp_rot):
        ...
        # vp_features = vp_features.permute(0, 2, 1, 3).contiguous().view(B*num_seed*num_depth, 256, self.nsample)  # (B*num_seed*num_depth, C, K)
        # vp_features = self.local_att(vp_features).contiguous().view(B, num_seed*num_depth, 256, self.nsample).permute(0, 2, 1, 3)
